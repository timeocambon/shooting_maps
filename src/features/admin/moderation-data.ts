...
export async function getAdminDashboardCounts() {
  ...
  const [proposals, reports, spots, withdrawals, photographers, reviews] = await Promise.all([
    ...
    supabase.from("photographers").select("id", { count: "exact", head: true }).eq("publication_state", "pending"),
    supabase.from("photographer_reviews").select("id", { count: "exact", head: true }).eq("moderation_state", "pending"),
  ]);
  ...
  return {
    ...
    pendingPhotographers: photographers.count ?? 0,
